...
category_map_rope3d = {"car": "Car", "van": "Car", "truck": "Car", "bus": "Car", "pedestrian": "Pedestrian", "bicycle": "Cyclist", "trailer": "Cyclist", "motorcycle": "Cyclist"}
...
def result2kitti(results_file, results_path, dair_root, gt_label_path, demo=False):
    ...
    for sample_token in results.keys():
        ...
        for pred in preds:
            ...
            if detection_score > 0.45 and class_name in category_map_dair.keys():
                ...
                bboxes.append(box)
        os.makedirs(os.path.join(results_path, "data_dairi"), exist_ok=True)
        write_kitti_in_txt(pred_lines, os.path.join(results_path, "data_dairi", "{:06d}".format(sample_id) + ".txt"))       
        if demo:
    ...
    return os.path.join(results_path, "data_dairi")
    ...
    for sample_token in tqdm(results.keys()):
        ...
        for pred in preds:
            ...
            if detection_score > 0.45 and class_name in category_map_rope3d.keys():
                ...
                pred_lines.append(line)
                bboxes.append(box)
        os.makedirs(os.path.join(results_path, "data_rope3d"), exist_ok=True)
        write_kitti_in_txt(pred_lines, os.path.join(results_path, "data_rope3d", "{:06d}".format(sample_id) + ".txt"))       
        if demo:
            os.makedirs(os.path.join(results_path, "demo"), exist_ok=True)
            label_path = os.path.join(gt_label_path, "{:06d}".format(sample_id) + ".txt")
            demo_file = os.path.join(results_path, "demo", "{:06d}".format(sample_id) + ".jpg")
            pcd_vis(bboxes, demo_file, label_path)
    return os.path.join(results_path, "data_rope3d")