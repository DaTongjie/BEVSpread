...
class RoadSideEvaluator():
    ...
    def evaluate(
        ...
        if 'dair' in self.data_root:
            pred_label_path = result2kitti(result_files["img_bbox"], self.results_path, self.data_root, self.gt_label_path, demo=False)
        else:
            pred_label_path = result2kitti_rope3d(result_files["img_bbox"], self.results_path, self.data_root, self.gt_label_path, demo=False)
        kitti_evaluation(pred_label_path, self.gt_label_path, current_classes=self.current_classes, metric_path=osp.join(self.output_dir, 'metrics'))
