...
from models.bev_spread import BEVSpread
...
results_path = "/workspace/mnt/storage/guangcongzheng/BEV_OUTPUT_1"
...
class BEVSpreadLightningModel(LightningModule):
    ...
    def __init__(self,
        ...
        self.model = BEVSpread(self.backbone_conf, self.head_conf)
        self.mode = 'valid'
    ...
    def configure_optimizers(self):
        lr = self.basic_lr_per_img * \
            self.batch_size_per_device * self.gpus * self.trainer.accumulate_grad_batches
...
def main(args: Namespace) -> None:
    ...
    model = BEVSpreadLightningModel(**vars(args))
    checkpoint_callback = ModelCheckpoint(dirpath='./outputs/bev_spread_r101_256x256_100m/checkpoints', filename='{epoch}', every_n_epochs=1, save_last=False, save_top_k=-1)
...
def run_cli():
    ...
    parser = BEVSpreadLightningModel.add_model_specific_args(parent_parser)
    parser.set_defaults(
        profiler='simple',
        deterministic=False,
        max_epochs=50,
        ...
        precision=32,
        default_root_dir='./outputs/bev_spread_r101_256x256_100m')
